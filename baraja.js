...
function iniciarMayorMenor() {
  ...
  function barajaSinPalo(baraja, paloExcluido) {
    return baraja.filter(carta => !carta.startsWith(paloExcluido + '-'));
  }


  function generarCarta(baraja, cartaAnterior, probabilidadRepetirCarta = 0.01, probabilidadRepetirPalo = 0.10, probabilidadRepetirValor = 0.01) {
    let nueva;
    do {
      nueva = baraja[Math.floor(Math.random() * baraja.length)];

      const [paloAnterior, valorAnterior] = cartaAnterior.split('-');
      const [paloNuevo, valorNuevo] = nueva.split('-');

      const mismoValor = valorNuevo === valorAnterior;
      const mismoPalo = paloNuevo === paloAnterior;
      const mismaCarta = nueva === cartaAnterior;

      if (mismaCarta && Math.random() > probabilidadRepetirCarta) continue;
      if (mismoPalo && Math.random() > probabilidadRepetirPalo) continue;
      if (mismoValor && Math.random() > probabilidadRepetirValor) continue;
  ...
  document.getElementById('btn-pedir-mm').addEventListener('click', () => {
    const cartaAnteriorId = historial[historial.length - 1];
    const paloAnterior = cartaAnteriorId.split('-')[0];
    const barajaFiltrada = barajaSinPalo(baraja, paloAnterior);
    const nueva = generarCarta(barajaFiltrada, cartaAnteriorId);
...
`;
  ...
  document.getElementById('btn-pedir-bj').addEventListener('click', () => {
    let carta;
    if (mano.length === 0) {
      // Primera tirada: sin restricciones
      carta = baraja[Math.floor(Math.random() * baraja.length)];
    } else {
      // Tiradas siguientes: aplicar exclusión de palo y control de repetición
      const cartaAnteriorId = mano[mano.length - 1];
      const paloAnterior = cartaAnteriorId.split('-')[0];
      const barajaFiltrada = barajaSinPalo(baraja, paloAnterior);
      carta = generarCarta(barajaFiltrada, cartaAnteriorId);
    }
